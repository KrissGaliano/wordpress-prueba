...
if ( ! function_exists( 'printo_storage_get' ) ) {
	/**
	 ...
	 */
	function printo_storage_get( $var_name, $default = '' ) {
		global $PRINTO_STORAGE;
		return isset( $PRINTO_STORAGE[ $var_name ] ) ? $PRINTO_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'printo_storage_set' ) ) {
	/**
	 ...
	 */
	function printo_storage_set( $var_name, $value ) {
		global $PRINTO_STORAGE;
		$PRINTO_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'printo_storage_empty' ) ) {
	/**
	 ...
	 */
	function printo_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $PRINTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $PRINTO_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $PRINTO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'printo_storage_isset' ) ) {
	/**
	 ...
	 */
	function printo_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $PRINTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $PRINTO_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $PRINTO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'printo_storage_unset' ) ) {
	/**
	 ...
	 */
	function printo_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $PRINTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $PRINTO_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $PRINTO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'printo_storage_inc' ) ) {
	/**
	 ...
	 */
	function printo_storage_inc( $var_name, $value = 1 ) {
		global $PRINTO_STORAGE;
		if ( empty( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = 0;
		}
		$PRINTO_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'printo_storage_concat' ) ) {
	/**
	 ...
	 */
	function printo_storage_concat( $var_name, $value ) {
		global $PRINTO_STORAGE;
		if ( empty( $PRINTO_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'printo_storage_get_array' ) ) {
	/**
	 ...
	 */
	function printo_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $PRINTO_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $PRINTO_STORAGE[ $var_name ][ $key ] ) ? $PRINTO_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'printo_storage_set_array' ) ) {
	/**
	 ...
	 */
	function printo_storage_set_array( $var_name, $key, $value ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PRINTO_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'printo_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function printo_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $PRINTO_STORAGE[ $var_name ][ $key ] ) ) {
			$PRINTO_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$PRINTO_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$PRINTO_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'printo_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function printo_storage_merge_array( $var_name, $key, $value ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PRINTO_STORAGE[ $var_name ] = array_merge( $PRINTO_STORAGE[ $var_name ], $value );
		} else {
			$PRINTO_STORAGE[ $var_name ][ $key ] = array_merge( $PRINTO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'printo_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function printo_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			printo_array_insert_after( $PRINTO_STORAGE[ $var_name ], $after, $key );
		} else {
			printo_array_insert_after( $PRINTO_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'printo_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function printo_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			printo_array_insert_before( $PRINTO_STORAGE[ $var_name ], $before, $key );
		} else {
			printo_array_insert_before( $PRINTO_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'printo_storage_push_array' ) ) {
	/**
	 ...
	 */
	function printo_storage_push_array( $var_name, $key, $value ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $PRINTO_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $PRINTO_STORAGE[ $var_name ][ $key ] ) ) {
				$PRINTO_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $PRINTO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'printo_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function printo_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $PRINTO_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $PRINTO_STORAGE[ $var_name ] ) && is_array( $PRINTO_STORAGE[ $var_name ] ) && count( $PRINTO_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $PRINTO_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $PRINTO_STORAGE[ $var_name ][ $key ] ) && is_array( $PRINTO_STORAGE[ $var_name ][ $key ] ) && count( $PRINTO_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $PRINTO_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'printo_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function printo_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRINTO_STORAGE[ $var_name ][ $key ] ) ) {
			$PRINTO_STORAGE[ $var_name ][ $key ] = 0;
		}
		$PRINTO_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'printo_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function printo_storage_concat_array( $var_name, $key, $value ) {
		global $PRINTO_STORAGE;
		if ( ! isset( $PRINTO_STORAGE[ $var_name ] ) ) {
			$PRINTO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRINTO_STORAGE[ $var_name ][ $key ] ) ) {
			$PRINTO_STORAGE[ $var_name ][ $key ] = '';
		}
		$PRINTO_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'printo_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function printo_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $PRINTO_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRINTO_STORAGE[ $var_name ] ) ? $PRINTO_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRINTO_STORAGE[ $var_name ] ) ? $PRINTO_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'printo_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function printo_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $PRINTO_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $PRINTO_STORAGE[ $var_name ]->$prop ) ? $PRINTO_STORAGE[ $var_name ]->$prop : $default;
	}
}
