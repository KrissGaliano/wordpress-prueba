...
if ( function_exists( 'printo_elementor_is_preview' ) && printo_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			printo_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	printo_new_main_query(
		array(
			'post_type'      => printo_get_theme_option( 'post_type' ),
			'category'       => printo_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'printo_filter_get_template_part', printo_blog_archive_get_template() ) );
}
