...
jQuery( document ).ready(
	function() {
		...
		printo_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="printo_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.printo_post_editor' );
						if ( link.length > 0 ) {
							printo_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.printo_post_editor', function(e) {
						printo_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('printo_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function printo_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
