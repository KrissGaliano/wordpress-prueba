...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			printo_create_observer( 'printo-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				printo_gutenberg_editor_init();
			} );
		} else {
		 	printo_gutenberg_editor_init();
		}
	...
	function printo_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'printo_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'printo_inited' ) ) {
	...
	// Init on page load
	function printo_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = printo_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + PRINTO_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + PRINTO_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in PRINTO_STORAGE['override_classes'] ) {
				$obj.addClass( PRINTO_STORAGE['override_classes'][i].replace( '%s', PRINTO_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( printo_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && PRINTO_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'printo_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			printo_remove_observer( 'printo-check-editor-styles-wrapper' );
			printo_create_observer( 'printo-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'printo_inited' );
	}
} );
