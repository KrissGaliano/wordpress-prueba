...
jQuery( document ).on( 'action.ready_printo', function() {
	...
	$document.on( 'action.resize_printo', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.printo_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		printo_set_cookie( 'printo_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	printo_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function printo_woocommerce_add_inc_dec() {
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited),.variations_form.cart div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					printo_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function printo_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_printo', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < printo_window_scroll_top() + printo_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		printo_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					printo_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					printo_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		printo_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			printo_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function printo_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = printo_window_width() + ( printo_window_height() < $document.height() || printo_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= PRINTO_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
